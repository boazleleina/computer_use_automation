...
| 3.4 | Allowlist of routes and action types; risky vs reversible handled conservatively; never persist secrets or raw PII | `domain/policy.py` | done; three layers — action allowlist, `may_operate` on the screen, `SECRET_CONTROL` on the control. Irreversible gated per invocation. **Mutating flow never discovered live** — exercised by tests only |
